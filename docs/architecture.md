...
scalar per cell). `analysis.field.field_map` folds the reweighted
...
(energy -> temperature) do. For a field, `sum2`/`sum4` and the per-pixel
Binder cumulant are recorded but unused (`field_map` reads only `_mean`); they
serve scalar order parameters, where they give the susceptibility and the
Binder crossing used to locate `T_c`.

Because a bin's `count` is shared across all S pixels, the recorder drops an
observation whole when any element is non-finite rather than element-wise; see
`EnergyBinnedObservableRecorder.record` for why a partial record would corrupt
the per-pixel mean.
